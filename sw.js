...
<meta content="Portale di filosofia antica, moderna e contemporanea, con approfondimenti su autori e correnti." name="description"/>
<!-- Favicon -->
...
/* Fallback se la <i> è vuota (niente FA) */
...
/* X nativa di Safari: resa visibile e “oro” */
...
/* (Se nel markup c’è anche un bottone .clear-search) – stesso allineamento della X nativa */
...
/* Se vuoi farlo apparire senza JS quando c’è testo (richiede l’HTML: input prima, poi .clear-search) */
...
<style id="patch-ui">/* CSS minimized for reliability */
      ...
      #russell-box{position:fixed;bottom:8vh;left:2rem;z-index:2147483602;background:rgba(10,10,30,.8);border:1px solid rgba(255,215,0,.25);border-radius:.62rem;padding:.62rem .75rem;max-width:24rem;box-shadow:0 .5rem 1.12rem rgba(0,0,0,.35)}
      #russell-btn{all:unset;cursor:pointer;display:block;color:#ffd86a}#russell-hint{position:fixed;bottom:19vh;left:3.5rem;z-index:2;color:#cfe8ff;opacity:.85;font-style:italic}
...
<style id="search-panel-css">
      ...
      #search-panel{position:fixed;inset:12% 12% auto 12%;min-height:16rem;max-height:70vh;background:rgba(10,10,30,.96);
...
<script id="search-config">
  // prova prima l’URL del tuo dominio, poi i fallback relativi
  window.SEARCH_INDEX_URLS = [
    'https://www.echidisofia.org/search-index.json?v=20251007',
    'search-index.json?v=20251007',
    '/assets/search-index.json?v=20251007'
  ];
</script>
<style id="search-icon-reactivate">
  /* riaccendi la lente e rendila cliccabile */
  .search-container .search-icon{
...
<style id="search-icon-css-fix">
  /* Il wrapper deve essere il riferimento per il posizionamento */
  .search-container{
    position: relative;
    overflow: visible;   /* niente clip */
    z-index: 3001;       /* sopra altri overlay vicini */
  }

  /* Aumenta lo spazio interno a destra:
     - ~18–20px per la X (Safari)
     - ~18–20px per la lente
     - margine di respiro */
  #site-search{
    padding-right: 3.8rem;  /* regola 3.4–4.2rem se serve */
    position: relative;
    z-index: 3002;
  }

  /* Lente assoluta dentro il box,
     posizionata a SINISTRA della X di cancellazione */
  .search-container .search-icon{
    position: absolute;
    right: 2.2rem;          /* ← la X sta più a destra; qui non si sovrappone */
    top: 50%;
    transform: translateY(-50%);
    font-size: 1.05rem;     /* regola la dimensione */
    line-height: 1;
    color: var(--gold, #d4af37);  /* visibile su sfondo scuro */
    opacity: .95;
    pointer-events: none;   /* decorativa: non cattura click */
    z-index: 3003;
  }

  /* FALLBACK: se Font Awesome non è caricato e l’<i> è “vuoto” */
  .search-container .search-icon:empty::before{
    content: "🔎";
    display: inline-block;
  ...
  /* (Già fatto in precedenza) X di cancellazione color oro su Safari */
  #site-search::-webkit-search-cancel-button{
    -webkit-appearance: none;
    height: 18px; width: 18px; opacity: 1;
    background-color: var(--gold, #d4af37);
...
<style id="search-cancel-restore">
  /* Ripristina il comportamento nativo del cancel button */
  #site-search::-webkit-search-cancel-button{
    -webkit-appearance: searchfield-cancel-button !important; /* ← torna nativo */
    background: none !important;      /* annulla la mask precedente */
    -webkit-mask: none !important;

    /* Tinta “gold-ish” senza perdere il click */
    filter: sepia(1) saturate(5.5) hue-rotate(345deg) brightness(1.05) contrast(1.05);
    opacity: 1;
  }

  /* Lascia spazio a lente + X (come prima) */
  #site-search{ padding-right: 3.8rem; }
  .search-container .search-icon{ right: 2.2rem; } /* la lente resta prima della X */
</style>
<style id="search-clear-btn">
  ...
  #site-search{
    padding-right: 3.8rem;   /*  X a destra + lente un po' più a sinistra */
  ...
  /* Nascondi la ❌ nativa (così non fa confusione) */
  #site-search::-webkit-search-cancel-button{
...
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24">\
  ...
  /* Mostra il bottone se c’è testo (CSS-only sui browser che supportano :has) */
...
<style id="cookie-consent-css">
  ...
  /* Banner compatto “fluttuante” (bottom center) */
  .cc-banner{
  ...
  /* FAB (biscottino) — lo muovo se “tocca” il box di Russell */
  #cookie-fab{
...
<style id="cookie-fab-label">
  ...
  #cookie-fab{
    ...
    align-items: center;
    gap: .45rem;                 /* distanza tra 🍪 e “Cookie” */
    height: 48px;
    ...
    font-weight: 700;            /* testo un po’ più deciso */
    /* mantiene colori/ombra già definiti in precedenza */
  }
  ...
  /* Tweak responsive: pill leggermente più compatto su schermi stretti */
...
<style id="floating-quotes-v2-css">
/* === Floating Quotes v2 (robust) === */
.floating-quotes{ position:fixed; bottom:20px; right:20px; z-index:2000; max-width: 360px; pointer-events:auto; }
.floating-quotes .quote-bubble{
  opacity: 0;
...
.floating-quotes .quote-bubble.show{
  opacity: 1;
  transform: translateX(0);
  z-index: 1;
}
.floating-quotes .quote-text{ line-height:1.35; }
@media (max-width: 480px){
  .floating-quotes{ right:12px; bottom:12px; max-width: 280px; }
}
</style><style id="quote-slide-css">
/* === Quote slide to center (restore) === */
#quote-runner{
  position: fixed;
  left: 50%;
  bottom: 260px;
...
#quote-runner .quote-bubble.slide-in{
  animation: quoteSlideInOut 9s ease-in-out forwards;
}
...
#quote-runner .quote-text{ font-size: 1.05rem; line-height: 1.4; }
#quote-runner .quote-author{ margin-top: .25rem; }
@media (max-width: 600px){
  #quote-runner{ bottom: 12px; }
  #quote-runner .quote-text{ font-size: 1rem; }
}
</style><style id="timeline-scroll-anim-v2">
/* === Timeline scroll animation v2 (restore) === */
.timeline-container .timeline-item{
  opacity: 0;
...
.timeline-container .timeline-item.left  { transform: translateX(-140px) scale(.96); }
.timeline-container .timeline-item.right { transform: translateX(140px)  scale(.96); }
...
.timeline-container .timeline-item.visible.left  { animation-name: tlSlideInLeft;  }
.timeline-container .timeline-item.visible.right { animation-name: tlSlideInRight; }
@keyframes tlSlideInLeft{
  0%   { transform: translateX(-160px) scale(.96); opacity: 0; filter: blur(8px); }
  55%  { transform: translateX(14px)  scale(1.015); opacity: 1; filter: blur(0); }
  75%  { transform: translateX(-8px)  scale(.997); }
...
</style><style id="timeline-left-right-layout">
/* === Timeline: enforce left/right columns (restore) === */
.timeline-container .timeline{ position: relative; }
.timeline-container .timeline-item{ position: relative; width: 50%; box-sizing: border-box; }
.timeline-container .timeline-item.left  { left: 0 !important; }
.timeline-container .timeline-item.right { left: 50% !important; }
@media (max-width: 980px){ .timeline-container .timeline-item{ width: 100%; left: 0 !important; } }
</style><style id="ultimate-clickthrough-zfix-2025-10-08">
/* === Ultimate: quotes must not block clicks; floating list hidden === */
#quote-runner, #quote-runner *{ pointer-events:none !important; }
#quote-runner{ z-index: 2200 !important; }
.floating-quotes{ display:none !important; pointer-events:none !important; z-index:1100 !important; }
</style><style id="search-lens-pointer-2025-10-08">
/* Ensure search lens is clickable above decorative layers */
.search-container .search-icon{ pointer-events:auto !important; cursor:pointer; position:relative; z-index:3003; }
.search-container{ position: relative !important; }
</style><style id="quote-runner-css-solid-2025-10-08">
#quote-runner{ position:fixed; left:50%; bottom:260px; transform:translateX(-50%); z-index:3000; pointer-events:none; max-width:min(86vw,680px); }
...
</style><style id="zfix-submenu-overlays-2025-10-08">
/* === Z-INDEX FIX: keep submenu above right-side boxes (articles/dictionary/encyclopedia) === */
.menu-container{ z-index: 2147483604 !important; } /* above .articles-blob/.right-embed (3600) */
nav.menu-desktop, nav.menu-desktop ul, nav.menu-desktop li{ position: relative; z-index: 2147483605 !important; }
...
@media (min-width: 981px){
  ...
  "inLanguage":"it-IT",
  "description":"Portale di filosofia antica, moderna e contemporanea",
  "publisher":{"@type":"Organization","name":"Echi di Sofia"}
...
<img alt="Socrate" decoding="async" height="340" loading="eager" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/Socrates_Louvre.jpg" width="260"/>
<p>Socrate</p>
</div>
<div class="slide-vertical">
<img alt="Platone" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/platone.png" width="260"/>
<p>Platone</p>
</div>
<div class="slide-vertical">
<img alt="Aristotele" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/Aristotle_.jpg" width="260"/>
<p>Aristotele</p>
</div>
<div class="slide-vertical">
<img alt="Maria Teresa" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/maria_teresa.jpg" width="260"/>
<p>🕯️Maria Teresa: virtù e bellezza in armonia✝️</p>
</div>
<div class="slide-vertical">
<img alt="Livia" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/Livia.jpeg" width="260"/>
<p>👑🦌Livia🦅DEA egizia🔱</p>
</div>
<div class="slide-vertical">
<img alt="Cartesio" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/cartesio.jpg" width="260"/>
<p>Cartesio</p>
</div>
<div class="slide-vertical">
<img alt="Kant" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/kant.webp" width="260"/>
<p>Kant - il più zoppo tra gli storpi del concetto</p>
</div>
<div class="slide-vertical">
<img alt="Gadamer" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/gadamer.webp" width="260"/>
<p>H.G.Gadamer</p>
</div>
<div class="slide-vertical">
<img alt="Polpetta" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/polpetta.webp" width="260"/>
<p>🏹 Polpetta🦸: guerriera celtica</p>
</div>
<div class="slide-vertical">
<img alt="Ricoeur" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/ricoeur.webp" width="260"/>
<p>P. Ricoeur</p>
</div>
<div class="slide-vertical">
<img alt="Levinas" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/levinas.webp" width="260"/>
<p>E. Lévinas</p>
</div>
<div class="slide-vertical">
<img alt="San Tommaso" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/s.Tommaso.jpg" width="260"/>
<p>San Tommaso</p>
</div>
<div class="slide-vertical">
<img alt="Cerbero" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/cerbero.webp" width="260"/>
<p>Cerbero - Guardiano dell'Ade</p>
</div>
<div class="slide-vertical">
<img alt="Nietzsche" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/friedrich-nietzsche.jpg" width="260"/>
<p>Nietzsche</p>
</div>
<div class="slide-vertical">
<img alt="Dea" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/dea.jpeg" width="260"/>
<p>𓍝Divinità mesopotamica⚯</p>
</div>
<div class="slide-vertical">
<img alt="Gianni Vattimo" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/vattimo.png" width="260"/>
<p>Gianni Vattimo</p>
</div>
<div class="slide-vertical">
<img alt="Testa di..." decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/krk.jpg" width="260"/>
<p>Testa di ...</p>
</div>
<div class="slide-vertical">
<img alt="Democrito" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/Democritus.jpg" width="260"/>
<p>Democrito</p>
</div>
<div class="slide-vertical">
<img alt="Talete" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/Talete.jpg" width="260"/>
<p>Talete</p>
</div>
<div class="slide-vertical">
<img alt="Aristippo" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/aristippo.webp" width="260"/>
<p>Aristippo</p>
</div>
<div class="slide-vertical">
<img alt="Massimo Cacciari" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/cacciari.jpg" width="260"/>
<p>Massimo Cacciari</p>
</div>
<div class="slide-vertical">
<img alt="Cicerone" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/cicerone.jpg" width="260"/>
<p>Cicerone</p>
</div>
<div class="slide-vertical">
<img alt="Dante Alighieri" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/dante.jpg" width="260"/>
<p>Dante Alighieri - la iena che fa poesia nelle tombe</p>
</div>
<div class="slide-vertical">
<img alt="Gilles Deleuze" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/deleuze.jpg" width="260"/>
<p>Gilles Deleuze</p>
</div>
<div class="slide-vertical">
<img alt="Jacques Derrida" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/derrida.jpg" width="260"/>
<p>Jacques Derrida</p>
</div>
<div class="slide-vertical">
<img alt="John Dewey" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/dewey.jpg" width="260"/>
<p>John Dewey</p>
</div>
<div class="slide-vertical">
<img alt="Michel Foucault" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/foucault.jpg" width="260"/>
<p>Michel Foucault</p>
</div>
<div class="slide-vertical">
<img alt="G.W.F. Hegel" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/hegel.avif" width="260"/>
<p>G.W.F. Hegel</p>
</div>
<div class="slide-vertical">
<img alt="Martin Heidegger" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/heidegger.jpg" width="260"/>
<p>Martin Heidegger</p>
</div>
<div class="slide-vertical">
<img alt="Max Horkheimer" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/horkheimer.jpg" width="260"/>
<p>Max Horkheimer</p>
</div>
<div class="slide-vertical">
<img alt="Edmund Husserl" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/husserl.jpg" width="260"/>
<p>Edmund Husserl</p>
</div>
<div class="slide-vertical">
<img alt="Maurice Merleau-Ponty" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/merleau-ponty.avif" width="260"/>
<p>Maurice Merleau-Ponty</p>
</div>
<div class="slide-vertical">
<img alt="Charles S. Peirce" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/peirce.jpg" width="260"/>
<p>Charles S. Peirce</p>
</div>
<div class="slide-vertical">
<img alt="Pirrone" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/pirrone.jpg" width="260"/>
<p>Pirrone</p>
</div>
<div class="slide-vertical">
<img alt="Pitagora" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/pitagora.avif" width="260"/>
<p>Pitagora</p>
</div>
<div class="slide-vertical">
<img alt="Richard Rorty" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/richard_rorty.jpg" width="260"/>
<p>Richard Rorty</p>
</div>
<div class="slide-vertical">
<img alt="Bertrand Russell" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/russell.jpeg" width="260"/>
<p>Bertrand Russell</p>
</div>
<div class="slide-vertical">
<img alt="Emanuele Severino" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/Severino.png" width="260"/>
<p>Emanuele Severino</p>
</div>
<div class="slide-vertical">
<img alt="Michail Bulgakov" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/bulgakov.jpg" width="260"/>
<p>Michail Bulgakov</p>
</div>
<div class="slide-vertical">
<img alt="Fëdor Dostoevskij" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/dostoevskij.gif" width="260"/>
<p>Fëdor Dostoevskij</p>
</div>
<div class="slide-vertical">
<img alt="Umberto Galimberti" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/galimberti.png" width="260"/>
<p>Umberto Galimberti</p>
</div>
<div class="slide-vertical">
<img alt="Max Weber" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/max%20weber.jpg" width="260"/>
<p>Max Weber</p>
</div>
<div class="slide-vertical">
<img alt="Lev Šestov" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/sestov.jpg" width="260"/>
<p>Lev Šestov</p>
</div>
<div class="slide-vertical">
<img alt="F.W.J. Schelling" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/schelling.jpg" width="260"/>
<p>F.W.J. Schelling</p>
</div>
<div class="slide-vertical">
<img alt="Ludwig Wittgenstein" decoding="async" height="340" loading="lazy" src="https://raw.githubusercontent.com/Casi0048/casi0048.github.io/main/immagini%20per%20sito/wittgenstein.jpg" width="260"/>
...
<label class="speed-label" for="slider-speed"><span class="speed-label-text">Regola velocità</span><input id="slider-speed" max="12" min="1" type="range" value="8"/></label>
</div>
<div aria-label="Lettore audio" class="audio-player" id="audio-player">
<div class="audio-title">🎧 Musica per lo studio</div>
<br/>
<div class="controls"><button id="playPause">▶️</button><span aria-live="polite" id="time">0:00 / 0:00</span><input id="volume" max="1" min="0" step="0.01" type="range" value="0.8"/></div>
...
<form id="dict-form">
<div class="row">
<input id="dict-q" placeholder="Cerca un termine…" type="search"/>
</div>
<div class="sources">
...
<nav aria-label="Menu principale" class="menu-desktop">
<ul role="menu">
<li class="dropdown">
...
<!-- Citazioni filosofiche fluttuanti -->
<div class="floating-quotes"><div class="quote-bubble"><div class="quote-text">“Conosci te stesso”</div><div class="quote-author">— Socrate</div></div><div class="quote-bubble"><div class="quote-text">“So di non sapere”</div><div class="quote-author">— Socrate</div></div><div class="quote-bubble"><div class="quote-text">“Una vita senza ricerca non è degna di essere vissuta”</div><div class="quote-author">— Socrate</div></div><div class="quote-bubble"><div class="quote-text">“La filosofia comincia con la meraviglia”</div><div class="quote-author">— Platone</div></div><div class="quote-bubble"><div class="quote-text">“La bellezza è lo splendore del vero”</div><div class="quote-author">— Platone</div></div><div class="quote-bubble"><div class="quote-text">“Siamo ciò che facciamo ripetutamente. L'eccellenza non è un atto ma un'abitudine”</div><div class="quote-author">— Aristotele</div></div><div class="quote-bubble"><div class="quote-text">“La felicità dipende da noi stessi”</div><div class="quote-author">— Aristotele</div></div><div class="quote-bubble"><div class="quote-text">“Nulla è nell'intelletto che prima non sia passato dai sensi”</div><div class="quote-author">— Aristotele</div></div><div class="quote-bubble"><div class="quote-text">“Tutto scorre”</div><div class="quote-author">— Eraclito</div></div><div class="quote-bubble"><div class="quote-text">“Il carattere è destino”</div><div class="quote-author">— Eraclito</div></div><div class="quote-bubble"><div class="quote-text">“L'uomo è misura di tutte le cose”</div><div class="quote-author">— Protagora</div></div><div class="quote-bubble"><div class="quote-text">“Vivere secondo natura”</div><div class="quote-author">— Zenone di Cizio</div></div><div class="quote-bubble"><div class="quote-text">“Non sono le cose che ci turbano, ma le opinioni che abbiamo su di esse”</div><div class="quote-author">— Epitteto</div></div><div class="quote-bubble"><div class="quote-text">“Non è la morte che dobbiamo temere, ma il non cominciare mai a vivere”</div><div class="quote-author">— Marco Aurelio</div></div><div class="quote-bubble"><div class="quote-text">“La vita è opinione”</div><div class="quote-author">— Marco Aurelio</div></div><div class="quote-bubble"><div class="quote-text">“La felicità consiste nel desiderare ciò che si ha”</div><div class="quote-author">— Seneca</div></div><div class="quote-bubble"><div class="quote-text">“Non esiste vento favorevole per il marinaio che non sa dove andare”</div><div class="quote-author">— Seneca</div></div><div class="quote-bubble"><div class="quote-text">“Il destino guida chi lo segue e trascina chi si oppone”</div><div class="quote-author">— Seneca</div></div><div class="quote-bubble"><div class="quote-text">“Vivi nascosto”</div><div class="quote-author">— Epicuro</div></div><div class="quote-bubble"><div class="quote-text">“Ama e fa' ciò che vuoi”</div><div class="quote-author">— Sant'Agostino</div></div><div class="quote-bubble"><div class="quote-text">“La fede cerca l'intelligenza”</div><div class="quote-author">— Anselmo d'Aosta</div></div><div class="quote-bubble"><div class="quote-text">“Contemplata aliis tradere”</div><div class="quote-author">— Tommaso d'Aquino</div></div><div class="quote-bubble"><div class="quote-text">“Non moltiplicare gli enti senza necessità”</div><div class="quote-author">— Guglielmo di Ockham</div></div><div class="quote-bubble"><div class="quote-text">“È meglio essere temuti che amati, se non si può essere entrambi”</div><div class="quote-author">— Niccolò Machiavelli</div></div><div class="quote-bubble"><div class="quote-text">“Cogito, ergo sum”</div><div class="quote-author">— Cartesio</div></div><div class="quote-bubble"><div class="quote-text">“Il cuore ha le sue ragioni che la ragione non conosce”</div><div class="quote-author">— Blaise Pascal</div></div><div class="quote-bubble"><div class="quote-text">“La mente è una tabula rasa”</div><div class="quote-author">— John Locke</div></div><div class="quote-bubble"><div class="quote-text">“Esse est percipi”</div><div class="quote-author">— George Berkeley</div></div><div class="quote-bubble"><div class="quote-text">“La bellezza delle cose esiste nella mente di chi le contempla”</div><div class="quote-author">— David Hume</div></div><div class="quote-bubble"><div class="quote-text">“Due cose riempiono l’animo: il cielo stellato sopra di me e la legge morale dentro di me”</div><div class="quote-author">— Immanuel Kant</div></div><div class="quote-bubble"><div class="quote-text">“Agisci in modo da trattare l’umanità sempre come fine e mai semplicemente come mezzo”</div><div class="quote-author">— Immanuel Kant</div></div><div class="quote-bubble"><div class="quote-text">“Quel che è reale è razionale”</div><div class="quote-author">— G.W.F. Hegel</div></div><div class="quote-bubble"><div class="quote-text">“La vita oscilla tra la noia e il dolore”</div><div class="quote-author">— Arthur Schopenhauer</div></div><div class="quote-bubble"><div class="quote-text">“La vita dev’essere compresa all’indietro, ma va vissuta in avanti”</div><div class="quote-author">— Søren Kierkegaard</div></div><div class="quote-bubble"><div class="quote-text">“Chi ha un perché per vivere può sopportare quasi ogni come”</div><div class="quote-author">— Friedrich Nietzsche</div></div><div class="quote-bubble"><div class="quote-text">“Diventa ciò che sei”</div><div class="quote-author">— Friedrich Nietzsche</div></div><div class="quote-bubble"><div class="quote-text">“Non esistono fatti, solo interpretazioni”</div><div class="quote-author">— Friedrich Nietzsche</div></div><div class="quote-bubble"><div class="quote-text">“Di ciò di cui non si può parlare si deve tacere”</div><div class="quote-author">— Ludwig Wittgenstein</div></div><div class="quote-bubble"><div class="quote-text">“Il significato è l’uso”</div><div class="quote-author">— Ludwig Wittgenstein</div></div><div class="quote-bubble"><div class="quote-text">“Il linguaggio è la casa dell’essere”</div><div class="quote-author">— Martin Heidegger</div></div><div class="quote-bubble"><div class="quote-text">“L’esistenza precede l’essenza”</div><div class="quote-author">— Jean‑Paul Sartre</div></div><div class="quote-bubble"><div class="quote-text">“Bisogna immaginare Sisifo felice”</div><div class="quote-author">— Albert Camus</div></div></div>
<!-- Footer -->
<footer>
...
<p>Tra pensiero e stelle: un percorso tra scuole, autori<br/> e temi della filosofia, dalla classicità al contemporaneo.</p>
</div>
...
<div>
<h4 class="cc-title">🍪 Utilizziamo i cookie</h4>
<p class="cc-text">
...
<button aria-label="Preferenze cookie" id="cookie-fab" title="Preferenze cookie">🍪 Cookie</button>
...
(function(){
  ...
  ready(function(){
    ...
    var i=0, j=0, del=false, pause=0;
    function loop(){
      var t = phrases[i];
      if(!del){
        if(j<=t.length){ el.textContent = t.slice(0, j++); }
        else { pause++; }
        if(pause>10){ del=true; pause=0; }
      }else{
        if(j>=0){ el.textContent = t.slice(0, j--); }
        else { del=false; i=(i+1)%phrases.length; }
      }
      setTimeout(loop, del?55:85);
    }
    loop();
  });
})();
...
document.addEventListener("DOMContentLoaded", function(){
  ...
  function scrollFallback(){
    ...
    items.forEach(function(el){
      if (!el.classList.contains('visible')){
        var r = el.getBoundingClientRect();
        if (r.top < vh*0.82 && r.bottom > 0){ el.classList.add('visible'); }
      }
    });
  }
...
</script><script id="quote-runner-js-solid-2025-10-08">
(function(){
  function ready(fn){ if(document.readyState!=='loading') fn(); else document.addEventListener('DOMContentLoaded', fn); }
  ...
  ready(function(){
    ...
    function readSource(){
      ...
      return list.map(function(q){
        var qt = q.querySelector('.quote-text'), qa = q.querySelector('.quote-author');
        return {t: (qt?qt.textContent.trim():''), a: (qa?qa.textContent.trim():'')};
      });
    }
    var data = readSource();
    if (!data.length){
    ...
    function render(item){
      runner.innerHTML='';
      var b=document.createElement('div'); b.className='quote-bubble slide-in';
      var qt=document.createElement('div'); qt.className='quote-text'; qt.textContent=item.t;
      var qa=document.createElement('div'); qa.className='quote-author'; qa.textContent=item.a;
      b.appendChild(qt); b.appendChild(qa); runner.appendChild(b);
    }
    function next(){
      render(data[idx]);
